...
def savings(gross_pay, tax_rate, expenses):
    ...
    tax_deduc = int(gross_pay * tax_rate)
    net_pay = gross_pay - tax_deduc
    takehome_pay = int(net_pay - expenses)
    return takehome_pay
...
def interest(principal, rate, periods):
    ...
    interest_value = int(principal*(rate*periods) + principal)
...
def body_mass_index(weight, height):
    ...
    height_foot = height[0]*0.3048
    height_inches = height[1]*0.0254
    
    bmi_thingy = weight_metric/((height_foot+height_inches)**2)
    return bmi_thingy