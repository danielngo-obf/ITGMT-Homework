...
def shift_by_letter(letter, letter_shift):
    ...
    alphabet = range(65,90+1)
    shiftDict = {'A':0, 'B':1, 'C':2, 'D':3, 'E':4, 'F':5, 'G':6, 'H':7, 'I':8, 'J':9, 'K':10,
                'L':11, 'M':12, 'N':13, 'O':14, 'P':15, 'Q':16, 'R':17, 'S':18, 'T':19, 'U':20,
                'V':21, 'W':22, 'X':23, 'Y':24, 'Z':25}
    letterInput = ord(letter)
    letterShift = shiftDict[letter_shift]
    
    for i in alphabet:
        newLetter = letterInput + letterShift
        ...
        if newLetter > 90:
            difference = newLetter - 90
            loopLetter = difference + 64
            
            return str(chr(loopLetter))
...
def scytale_decipher(message, shift):
    ...
    # for context, this code was modified with the help of ChatGPT given the code from scytale_cipher
    # this is the only code that utilized ChatGPT
    # it was created by prompting ChatGPT to use lists and indexing, which was the original method I utilized for the whole homework
